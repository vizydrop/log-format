...
describe(`integration`, () => {
    ...
    describe(`logging cases`, () => {
        ...
        it(`should log fields with unstable type as String`, () => {
            const transport = new TestTransport();
            ...
            expect(transport.lastLog[`@fields`]).toEqual({
                level: `info`,
                stack: expect.any(String),
                __errorMessage: `ENOENT`,
                errno: `-2`,
            });
            expect(transport.lastLog[`@message`]).toBe(`ENOENT`);
            expect(transport.lastLog[`@timestamp`]).toBeTruthy();
        });
    ...
    describe(`timer`, () => {
        it(`should work with correlation id and additional fields`, async () => {
            const transport = new TestTransport();
            const logger = createLogger({
                mode: `production`,
                level: `info`,
            ...
            timer.done(`i'm done %s`, `today`, {a: 1});
            expect(transport.lastLog[`@message`]).toBe(`i'm done today`);
            expect(transport.lastLog[`@fields`]).toEqual({
                level: `info`,
                field1: `f1`,
                correlationId: `my1`,
            ...
            expect(transport.lastLog[`@fields`].durationMs).toBeGreaterThan(3);
            expect(transport.lastLog[`@fields`].durationMs).toBeLessThan(15);
            expect(transport.lastLog[`@timestamp`]).toBeTruthy();
        });
    });
});
