...
describe(`formatters`, () => {
            ...
            it(`should strip options from ECONREFUSED`, async () => {
                ...
                try {
                    await requestPromise.get(
                        `http://dskaldjlksadhjsakjhdjlkhsdkfjahfkjdhflkajhflkashfdsa.com`,
                    );
                } catch (err) {
    ...
    describe(`#addAdditionalFields`, () => {
        it(`should do nothing if additional fields are missing`, () => {
            const info = {a: 1, level: `error`};
            const result = addAdditionalFields({info});
            expect(result).toEqual(info);
        });

        it(`should add additional fields`, () => {
            const info = {a: 1, level: `error`};
            const result = addAdditionalFields({
                info,
                additionalFields: {test: 1},
            });
            expect(result).toEqual({
                ...info,
                test: 1,
