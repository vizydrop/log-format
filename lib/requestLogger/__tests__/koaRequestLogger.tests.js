...
describe(`koa request logger`, () => {
    ...
    it(`should log request`, async () => {
        ...
        expect(logger.dataContainer[0].data).toEqual({type: `inbound`});
        expect(logger.dataContainer[1].msg).toBe(
            `GET 200 /test?token=******** (${
                logger.dataContainer[1].data.size
            } bytes) ${logger.dataContainer[1].data.timeMs.toFixed(3)} ms`,
        );
        expect(logger.dataContainer[1].data).toEqual({
            type: `outbound`,
